...
mod configuration_tests {
    ...
    fn create_riscv_sv32_configuration_from_code() -> Result<()> {
        ...
        assert_eq!(machine.memory_regions.regions[0].start_address, 0);
        assert_eq!(machine.memory_regions.regions[0].end_address, 1);
        assert_eq!(
            machine.memory_regions.regions[1].region_type,
            MemoryRegionType::ROM
        );
        assert_eq!(machine.memory_regions.regions[1].start_address, 2);
        assert_eq!(machine.memory_regions.regions[1].end_address, 3);
        assert_eq!(
            machine.dumpfile,
